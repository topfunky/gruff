  ...
  def draw_bars
    ...
    @norm_data.each_with_index do |data_row, row_index|
      data_row[DATA_VALUES_INDEX].each_with_index do |data_point, point_index|
        ...
        temp1 = @graph_left + (@graph_width - data_point * @graph_width - length[point_index])
        temp2 = @graph_left + @graph_width - length[point_index]

        #Calculate the length of our bar, make sure its not negative.
        difference = temp2 - temp1
        difference = [difference, 0].max

        #Pick up the bar's color from our table's config
        @d = @d.fill data_row[DATA_COLOR_INDEX]

        #Calculate the four corners of our bar
        ...
        #Adjust the starting points for the next bar
        length[point_index] += difference
        height[point_index] += (data_point * @graph_width - 4)
